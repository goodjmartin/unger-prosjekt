...
public class SAMLSecurityConfig extends WebSecurityConfigurerAdapter implements InitializingBean, DisposableBean {
	...
	@Value("${security.saml.metadata}")
	private String metadataPath;

	@Value("${security.saml.keystore}")
	private String keystorePath;

	@Value("${security.saml.keystore.storePass}")
	private String keyStorePass;

	@Value("${security.saml.keystore.keyPass}")
	private String keyPass;

	@Value("${security.saml.keystore.alias}")
	private String keyAlias;

	@Value("${security.saml.spEntityId}")
	private String spEntityId;

	@Value("${security.saml.spBaseUrl}")
	...
	@Value("${security.url.main}")
	private String appUrl;

	// Default is 2 days
	@Value("${security.saml.maxAuthenticationAge:172800}")
	...
	public ExtendedMetadataDelegate ssoCircleExtendedMetadataProvider() throws MetadataProviderException {
		try {
			...
			final ClasspathResource classpathResource = new ClasspathResource(metadataPath);

			ResourceBackedMetadataProvider  resourceBackedMetadataProvider = new ResourceBackedMetadataProvider(this.backgroundTaskTimer, classpathResource);
	...
	public SavedRequestAwareAuthenticationSuccessHandler successRedirectHandler() {
		...
		successRedirectHandler.setDefaultTargetUrl(appUrl);
	...
	public SAMLProcessingFilter samlWebSSOProcessingFilter() throws Exception {
		...
		AuthenticationManager aa=null;
		try {
			aa = authenticationManager();
		}catch (Exception e){
		}
		samlWebSSOProcessingFilter.setAuthenticationManager(aa);
	...
	public SimpleUrlLogoutSuccessHandler successLogoutHandler() {
		...
		successLogoutHandler.setDefaultTargetUrl(logOutUrl);
	...
	public FilterChainProxy samlFilter() throws Exception {
		...
		chains.add(new DefaultSecurityFilterChain(new AntPathRequestMatcher(loginUrl + "/**"),
				samlEntryPoint()));
		chains.add(new DefaultSecurityFilterChain(new AntPathRequestMatcher(logOutUrl + "/**"),
				samlLogoutFilter()));
	...
	protected void configure(HttpSecurity http) throws Exception {
		LOGGER.info(String.format("SAMLSecurityConfig: enableSSL=%s, requiresSecureRegexMatchers=%s", enableSSL, requiresSecureRegexMatchers));
		...
		try {
			...
			http
					.authorizeRequests()
					//.antMatchers(GET, "/gmi-test").permitAll()
					.antMatchers("/").permitAll()
					.antMatchers("/saml/**").permitAll()
					.antMatchers("/VAADIN/**", "/PUSH/**", "/UIDL/**", "/vaadinServlet/**").permitAll()
